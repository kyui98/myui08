$(function(){
  ...
  $(window).scroll(function() {
    ...
    let access_position = $('#access').offset().top - $(window).height() + 450; 
    ...
    /*追加*/
    $(window).scroll(function() {
      let scroll = $(window).scrollTop(); // 現在のスクロール位置を取得
      let gallery_position = $('#gallery').offset().top - $(window).height();
    ...
    $(window).on('scroll', function() {
      let scroll = $(window).scrollTop();
      let galleryTop = $('#gallery').offset().top - $(window).height() / 1.5; // トリガー条件を調整
...
function hamburger() {
  ...
  if ($('.hamburger').hasClass('active')) {
    // hamburgerクラスにactiveクラスが存在する場合は、navにもactiveクラスを追加する
    $('#nav').addClass('active');
  } else {
    // hamburgerクラスにactiveクラスが存在しない場合は、navからactiveクラスを削除する
